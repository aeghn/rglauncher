...
;
        let extra = gtk::Grid::builder().hexpand(true).vexpand(false).valign(End).build();        // preview.attach(&extra, 0, 3, 1, 1);
        ...
        let sw = gtk::ScrolledWindow::builder().vexpand(true).hexpand(true).build();
        sw.set_child(Some(&b));

        let tb = gtk::Box::builder().vexpand(true).hexpand(true).orientation(Orientation::Vertical).build();
